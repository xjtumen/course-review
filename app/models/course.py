...
class Course(db.Model):
    ...
    reviews = db.relationship('Review', backref=db.backref('course', lazy='joined'),
    ...
    _info_history = db.relationship('CourseInfoHistory', order_by='desc(CourseInfoHistory.update_time)',
                                    backref='course',
                                    lazy='dynamic')
    ...
    def short_link(self):
        return Markup('<a href="' + self.url + '">') + Markup.escape(
            self.name + self.teacher_names_bracket_short) + Markup(
            '</a>')
    ...
    def review_per_year_dist(self):
        return db.session.query(db.extract('year', Review.publish_time).label('year'), db.func.count(Review.id),
                                db.func.avg(Review.rate)).filter(Review.course_id == self.id).filter(
            Review.is_hidden == False).filter(Review.rate > 0).group_by(db.text('year')).order_by(
            db.text('year')).all()

    def review_per_term_dist(self):
        return db.session.query(Review.term, db.func.count(Review.id), db.func.avg(Review.rate)).filter(
            Review.course_id == self.id).filter(Review.is_hidden == False).filter(Review.rate > 0).group_by(
            Review.term).order_by(Review.term).all()
    ...
    def compute_normalized_rate(self, rate_total, review_count, avg_rate=None, avg_rate_count=None):
        ...
        if avg_rate_count is None:
            avg_rate_count = \
                db.session.query(db.func.count(Review.id) / db.func.count(db.func.distinct(Review.course_id))).first()[
                    0]
        normalized_rate = (rate_total + avg_rate * avg_rate_count) / (review_count + avg_rate_count)
        return normalized_rate
    ...
    def join(self, user=current_user):
        ...
        last_class = CourseClass.query.filter(CourseClass.course_id == self.id).order_by(
            CourseClass.term.desc()).first()
    ...
    def joined_users(self):
        ...
        return User.query.join(Student).join(join_course).join(CourseClass).filter(
