...
def get_chatgpt_summary(course, prompt):
  try:
    ...
    print(
      f"Get summary of course #{course.id}, prompt_length {prompt_length}, prompt_tokens {prompt_tokens}, completion_tokens {completion_tokens}, time {elapsed_time}:")
    print(completion)
    return completion
...
def get_summary_of_course(course):
  public_reviews = (Review.query.filter_by(course_id=course.id)
                    .filter(Review.is_hidden == False).filter(Review.is_blocked == False).filter(
    Review.only_visible_to_student == False)
...
def get_summary_of_all_courses():
  ...
  courses = Course.query.join(CourseRate).filter(Course.id == CourseRate.id).order_by(
    CourseRate.review_count.desc()).filter(CourseRate.review_count > 0).all()
