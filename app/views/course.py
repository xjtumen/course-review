...
join_types = ['全球视野与中国道路', '哲学智慧与创新思维', '文化传承与艺术审美', '生命关怀与社会认知',
              '科学探索与技术创新', '经典导读与学术写作']
...
def plot_row(plot_term):
    ...
    try:
        ...
        fig.add_vline(average, line_width=1, line_dash="dash", line_color='blue',
                      annotation_text=f'平均: {average:.1f}', annotation_position='top left')
        fig.add_vline(highest, line_width=1, line_dash="dash", line_color='orange',
                      annotation_text=f'最高: {highest:.0f}')
        if lowest != 0.:
            fig.add_vline(lowest, line_width=1, line_dash="dash", line_color='red',
                          annotation_text=f'最低: {lowest:.0f}', annotation_position='top left')
        else:
            fig.add_vline(50., line_width=1, line_dash="dash", line_color='red', annotation_text=f'最低: {lowest:.0f}')
...
def index():
    ...
    if sort_by == 'popular':
        ...
        courses_page = course_query.join(CourseRate).order_by(Course.QUERY_ORDER()).paginate(page=page,
                                                                                             per_page=per_page)
    ...
    return render_template('course-index.html', courses=courses_page,
                           ...
                           join_type=join_type, join_types=join_types,
...
def view_course(course_id):
    ...
    return render_template('course.html', course=course, course_rate=course.course_rate, reviews=reviews,
                           ...
                           description=str(course.rate.average_rate) + ' 分，' + str(
                               course.rate.review_count) + ' 人评价',
