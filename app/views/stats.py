...
def index(lang_en=False):
  ...
  site_stat['course_avg_rate'] = course_avg_rate = db.session.query(db.func.avg(Review.rate)).first()[0]
  if course_avg_rate is None:
    site_stat['course_avg_rate'] = 0.
  site_stat['course_avg_rate_count'] = course_avg_rate_count = \
    db.session.query(db.func.count(Review.id) / db.func.count(db.func.distinct(Review.course_id))).first()[0]
  if course_avg_rate_count is None:
    site_stat['course_avg_rate_count'] = 0.
  ...
  course_review_counts = db.session.query(func.count(Review.id).label('review_count')).group_by(
    Review.course_id).subquery()
  course_review_count_dist = db.session.query(db.text('review_count'), func.count().label('course_count')).select_from(
    course_review_counts).group_by(db.text('review_count')).order_by(db.text('review_count')).all()
  ...
  user_review_counts = (db.session.query(func.count(Review.id).label('review_count'))
                        .filter(Review.is_anonymous == False).filter(Review.is_hidden == False).filter(
    Review.is_blocked == False)
                        .group_by(Review.author_id).subquery())
  user_review_count_dist = db.session.query(db.text('review_count'), func.count().label('user_count')).select_from(
    user_review_counts).group_by(db.text('review_count')).order_by(db.text('review_count')).all()

  # find the distribution of publication dates of reviews (count per month)
  review_dates = (db.session.query(func.year(Review.publish_time).label('publish_year'),
                                   func.month(Review.publish_time).label('publish_month'),
                                   func.count().label('review_count'))
                  .filter(Review.is_hidden == False).filter(Review.is_blocked == False)
                  .group_by(db.text('publish_year'), db.text('publish_month')).order_by(db.text('publish_year'),
                                                                                        db.text('publish_month')).all())

  # find the distribution of registration dates of user (count per month)
  user_reg_dates = db.session.query(func.year(User.register_time).label('reg_year'),
                                    func.month(User.register_time).label('reg_month'),
                                    func.count().label('user_count')).group_by(db.text('reg_year'),
                                                                               db.text('reg_month')).order_by(
  ...
  course_rates = db.session.query(func.floor(CourseRate._rate_average).label('rate'),
                                  func.count(func.floor(CourseRate._rate_average)).label('count')).filter(
    CourseRate._rate_average > 0).group_by(db.text('rate')).order_by(db.text('rate')).all()

  template = 'en/site-stats.html' if lang_en else 'site-stats.html'
  return render_template(template, site_stat=site_stat, course_review_count_dist=course_review_count_dist,
                         user_review_count_dist=user_review_count_dist, review_dates=review_dates,
                         user_reg_dates=user_reg_dates, review_rates=review_rates, course_rates=course_rates,
                         date=today, title='站点统计')
...
def view_ranking():
  ...
  teacher_rank_join = sql.outerjoin(sql.join(Teacher, sql.join(course_teachers,
                                                               sql.join(Course, Review, Course.id == Review.course_id),
                                                               course_teachers.c.course_id == Course.id),
                                             course_teachers.c.teacher_id == Teacher.id), Dept,
                                    Dept.id == Course.dept_id)
  # find teachers with low rating courses (average rate < 8)
  teachers_with_low_rating_course = sql.select(Teacher.id).join(course_teachers).join(Course).join(CourseRate).filter(
    CourseRate._rate_average > 0).filter(CourseRate._rate_average < 8)
  # find teachers with at least 3 high rating courses (average rate > 9)
  teacher_query_with_high_rating_course = sql.select(Teacher.id.label('teacher_id'),
                                                     func.count(CourseRate.id).label('course_count')).select_from(
    sql.join(Teacher, sql.join(course_teachers, CourseRate, course_teachers.c.course_id == CourseRate.id),
             Teacher.id == course_teachers.c.teacher_id)).filter(CourseRate._rate_average > 9).group_by(Teacher.id)
  teachers_with_high_rating_course = db.session.query(db.text('teacher_id')).select_from(
    teacher_query_with_high_rating_course).filter(db.text('course_count >= 3')).all()
  ...
  teacher_rank = (
    db.session.query(db.text('teacher_id'), db.text('teacher_name'), db.text('dept_id'), db.text('course_count'),
                     db.text('review_count'),
                     Course.generic_query_order(db.text('total_review_rate'), db.text('review_count')).label(
                       'normalized_review_rate'))
  ...
  return render_template('ranking.html', topk_count=topk_count, default_show_count=default_show_count, stats=stats,
                         teachers=teacher_rank, users=user_rank, reviews=review_rank, long_reviews=review_length_rank,
                         top_rated_courses=top_rated_courses, worst_rated_courses=worst_rated_courses,
...
def stats_history(lang_en=False):
  ...
  site_stat['course_count'] = Course.query.distinct(Course.id).join(CourseTerm).filter(
    CourseTerm.term < date_to_term(date)).count()
  site_stat['review_count'] = Review.query.filter(Review.publish_time < date).count()
  site_stat['registered_teacher_count'] = User.query.filter(User.identity == 'Teacher').filter(
    User.register_time < date).count()

  first_user = User.query.order_by(User.register_time).limit(1).first()
  site_stat['running_days'] = (date - first_user.register_time).days

  # find the distribution of the number of reviews per course
  course_review_counts = db.session.query(func.count(Review.id).label('review_count')).filter(
    Review.publish_time < date).group_by(Review.course_id).subquery()
  course_review_count_dist = db.session.query(db.text('review_count'), func.count().label('course_count')).select_from(
    course_review_counts).group_by(db.text('review_count')).order_by(db.text('review_count')).all()
  ...
  review_dates = db.session.query(func.year(Review.publish_time).label('publish_year'),
                                  func.month(Review.publish_time).label('publish_month'),
                                  func.count().label('review_count')).group_by(db.text('publish_year'),
                                                                               db.text('publish_month')).filter(
    Review.publish_time < date).order_by(db.text('publish_year'), db.text('publish_month')).all()

  # find the distribution of registration dates of user (count per month)
  user_reg_dates = db.session.query(func.year(User.register_time).label('reg_year'),
                                    func.month(User.register_time).label('reg_month'),
                                    func.count().label('user_count')).group_by(db.text('reg_year'),
                                                                               db.text('reg_month')).filter(
    User.register_time < date).order_by(db.text('reg_year'), db.text('reg_month')).all()

  # find the distribution of review rates
  review_rates = db.session.query(func.count(Review.id).label('count'), Review.rate).filter(
    Review.publish_time < date).group_by(Review.rate).order_by(Review.rate).all()

  # find the distribution of course rates
  course_rate_subquery = db.session.query(func.floor(func.avg(Review.rate)).label('rate')).filter(
    Review.publish_time < date).group_by(Review.course_id)
  course_rates = db.session.query(db.text('rate'), func.count(db.text('rate')).label('count')).select_from(
    course_rate_subquery).group_by(db.text('rate')).order_by(db.text('rate')).all()

  template = 'en/site-stats.html' if lang_en else 'site-stats.html'
  return render_template(template, site_stat=site_stat, course_review_count_dist=course_review_count_dist,
                         user_review_count_dist=user_review_count_dist, review_dates=review_dates,
                         user_reg_dates=user_reg_dates, review_rates=review_rates, course_rates=course_rates,
                         date=date_str, title='站点统计历史')
